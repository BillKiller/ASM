data segment
	...
	string_version db 'Version 1.0$'
	string_author db 'By JACK $'
	string_information db 'Help poor children in Scau$'
	string_help1 db 'press keyboard i to  change into Insert mode','$'
 	string_help2 db 'press F1 to write command$'
 	string_help3 db 'w means write and q means quit$'

 	noNameFile db 'please save with a name$'
 	cr db 0dh,0ah,'$'
	isEnd DW 0
	isExit dw 0
	blank db '                             $'
...
show_about proc near
	...
	mov dl,35
	call setCursor
	lea dx,string_version
	call errm
	mov dh,11
	mov dl,27
	call setCursor
	lea dx,string_information
	call errm
	mov dh,12
	mov dl,36
	call setCursor
	lea dx,string_author
	call errm
	mov dh,13
	mov dl,18
	call setCursor
